#!/usr/bin/env python3
"""
Summarize Monte Carlo results from HD Linear simulations.
...
def load_results(results_dir: str, n_obs_list: list) -> dict:
    ...
    for n_obs in n_obs_list:
        filename = f'mc_results_hd_linear_n{n_obs}.csv'
...
def compute_summary_statistics(results_dict: dict) -> pd.DataFrame:
    ...
    desired_order = ['SGD', 'LARS', 'RMD', 'RMD-CV', 'PGMM', 'PGMM-CV', 'A-PGMM', 'A-PGMM-CV']
...
def format_table(summary_df: pd.DataFrame, n_obs_list: list) -> str:
    ...
    lines = []
    lines.append("=" * 100)
    lines.append("Monte Carlo Results Summary: High-Dimensional Linear Regression")
    lines.append("=" * 100)
...
def generate_latex_table(summary_df: pd.DataFrame, n_obs_list: list) -> str:
    ...
    lines.append("\\caption{Monte Carlo Results: High-Dimensional Linear Regression}")
...
def main():
    ...
    print("\n" + "=" * 80)
    print("Monte Carlo Results Summary Generator - HD Linear")
    print("=" * 80)
    ...
    table_file = output_dir / 'summary_hd_linear_table.txt'
    with open(table_file, 'w') as f:
        f.write(formatted_table)
    print(f"\nFormatted table saved to: {table_file}")

    # Save LaTeX table
    latex_file = output_dir / 'summary_hd_linear_table.tex'
    with open(latex_file, 'w') as f:
        f.write(latex_table)
    print(f"LaTeX table saved to: {latex_file}")

    # Save summary DataFrame as CSV
    csv_file = output_dir / 'summary_hd_linear_statistics.csv'
